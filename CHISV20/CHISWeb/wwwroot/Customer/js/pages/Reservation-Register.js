...
$(function () {
	 ...
	 */
    $.backstretch("/Customer/images/web_bg.jpg");
    ...
    $('#gh_search').click(function () {
        ...
        $.getJSON("/Customer/Reservation/Json_GetCustomerList", d, function (jn) {
            if (jn.rlt) {
                var fnum = jn.items.length;
                if (fnum == 0) {
                    $.confirm("询问", "用户信息不完善，是否完善信息?", function () { location.href = "/Customer/Reservation/Register"; })

                    //if (confirm("您输入的信息有误，或者您还不是会员，请先注册会员！")) { location.href = "/Customer/Reservation/Register" }
                } else if (fnum == 1) {
                    //直接选择门诊和医生
    ...
    $('.btn-reservation').click(function () {
        ...
        if (next_step) {
            ...
            $.confirm("确认", "是否真的预约就诊？", function () {
                $.loadJSON("/Customer/Reservation/Json_GetReservationInfo", d, function (jn) {
                    if (jn.rlt) {
                        //列表
    ...
    function getStationList(pageindex) {
        $.loadJSON("/Customer/Reservation/Json_GetStationList", { pageIndexSearch: pageindex }, function (jn) {
            //列表
            //模板与模板数据   
...
function GetDocsByDepartmemtId() {
    $.getJSON("/Customer/Reservation/Json_GetDoctorOfDepartment", { departmentId: $('#departmentSelect').val() }, function (jn) {
        $("#DocsSelect").empty().append("<option value=''>-- 选择医生 --</option>");
        $('#day-time').empty();
        $.each(jn.docList, function (i, item) {
            $("#DocsSelect").append("<option value='" + item.employeeID + "'>" + item.employeeName + "</option>");
        });
        loadDoctorWorkInfos();
    })
}
...
function changeDoctorTime() {
    ...
    $.each(pdata.WorkInfos, function (i, m) {
        if (m.isWork && m.employeeId == docId && m.workDate.substr(0, 10) == date) {
...
function changeDoctorTimeSlot() {
    ...
    //从后台获取当日医生的预约情况
    d.slotNum && $.loadJSON("/Customer/Reservation/Json_GetDoctorReservationInfo", d, function (jn) {
        if (jn.rlt) {
            if (!jn.isWorkSlot) {
...
function loadDoctorWorkInfos() {
    ...
    if (doctorId) {
        ...
        $.loadJSON("/Customer/Reservation/Json_GetDoctorWorkInfos", d, function (jn) {
            if (jn.rlt) {
...
function departmentIdByQuery(stationId) {
    ...
    $.getJSON("/Customer/Reservation/Json_departmemtIdByQuery", { stationId: stationId }, function (jn) {
        if (jn.rlt) {
    ...
    if (phonenum != null) {
        ...
        if ($.trim($('#Telephone').val()).length == 0) {
            ...
            if ($.f.isPhoneNo($.trim($('#Telephone').val())) == false) {
                ...
                $.getJSON("/Customer/Reservation/Json_SendMobCode", { mob: phonenum }, function (jn) {
                    if (jn.rlt) {
