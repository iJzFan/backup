...
$(function () {
	 ...
	 */
    $.backstretch("/v20/images/web_bg.jpg");
    ...
    $('#gh_search').click(function () {
        ...
        $.getJSON("/v20/Reservation/Json_GetCustomerList", d, function (jn) {
            if (jn.rlt) {
                var fnum = jn.items.length;
                if (fnum == 0) {
                    $.confirm("询问", "用户信息不完善，是否完善信息?", function () { location.href = "/v20/Reservation/Register"; })
    ...
    function getStationList(pageindex) {
        $.loadJSON("/v20/Reservation/Json_GetStationList", { pageIndexSearch: pageindex }, function (jn) {
            //列表
            //模板与模板数据   
...
function GetDocsByDepartmemtId() {
    $.getJSON("/v20/Reservation/Json_GetDoctorOfDepartment", { departmentId: $('#departmentSelect').val() }, function (jn) {
...
function changeDoctorTimeSlot() {
    ...
    //从后台获取当日医生的预约情况
    d.slotNum && $.loadJSON("/v20/Reservation/Json_GetDoctorReservationInfo", d, function (jn) {
        if (jn.rlt) {
            if (!jn.isWorkSlot) {
...
function loadDoctorWorkInfos() {
    ...
    if (doctorId) {
        ...
        $.loadJSON("/v20/Reservation/Json_GetDoctorWorkInfos", d, function (jn) {
            if (jn.rlt) {
...
function departmentIdByQuery(stationId) {
    ...
    $.getJSON("/v20/Reservation/Json_departmemtIdByQuery", { stationId: stationId }, function (jn) {
        if (jn.rlt) {
    ...
    if (phonenum != null) {
        ...
        if ($.trim($('#Telephone').val()).length == 0) {
            ...
            if ($.f.isPhoneNo($.trim($('#Telephone').val())) == false) {
                ...
                $.getJSON("/v20/Reservation/Json_SendMobCode", { mob: phonenum }, function (jn) {
                    if (jn.rlt) {
