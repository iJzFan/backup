...
$.fn.SelectArea = function (cmd, areaId) {
    ...
    if ($tagcontrol.val()) {
        //初始化地址
        $.getJSON("/openapi/Utils/JetAreasId", { areaId: $tagcontrol.val() }, function (jn) {
...
function zTreeObj(options) {
    if (options.isExpandAll == undefined) options.isExpandAll = true;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            treeObj.expandAll(options.isExpandAll);
...
(function ($) {
    $.fn.extend({
        ...
        singleSelector: function (options) {
            ...
            $c.on('keydown', '.search-panel .search-text', function (evt) {
                console.log(evt.keyCode);
                if (evt.keyCode == 13) {//Enter输入
                    search();
                    evt.stopPropagation();
                    return false;
                }
            ...
            $c.find('.select').click(function () {
                ...
                $c.find('.search-panel .search-text').val('').focus().select();
            });
