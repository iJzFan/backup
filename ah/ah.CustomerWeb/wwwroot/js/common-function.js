...
$.f = {
    ...
    addCSSFile: function (filename) {
        ...
        if (!$.f.isInclude(cssfile)) {
            //$("head").append('<link rel="stylesheet" href="/v20/lib/myWidget/myWidget.css" />');
...
function toPriceStr(obj) {
    if (isNaN(obj)) return Str(obj);
    return obj.toFixed(2);
}
//关注、取消关注 医生或者工作站
$.isFollow = function (stationId, doctorId, state, f,close) {
    var content = "是否确定关注？";
    if (!state) {
        content = "是否取消关注？";
    }
    $.confirm("询问", content, function () {
        var opt = {
            isFollow: state,
        }
        if (stationId != null) opt.stationId = stationId;
        if (doctorId != null) opt.doctorId = doctorId;
        $.post("/Customer/Follow", opt, function (jn) {
            if (jn.rlt) {
                $.ok("操作成功！")
                f();
            } else {
...
//关注、取消关注 医生或者工作站 (没有询问层)
$.isFollowNoConfim = function (stationId, doctorId, state, f) {
    var opt = {
        isFollow: state,
    }
    if (stationId != null) opt.stationId = stationId;
